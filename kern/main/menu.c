...
static int cmd_dbflagsmenu(int n, char **a);
...
common_prog(int nargs, char **args)
{
	...
	struct thread *user_process;

	int pid = get_new_pid();
	if(pid == -1)
	{
		kprintf("No more pid available! Can't start user process :'(\n");
		return EAGAIN;
	}
	int spl = splhigh();
	result = thread_fork(args[0] /* thread name */,
			args /* thread arg */, nargs /* thread arg */,
			cmd_progthread, &user_process);
	...
	user_process->is_user_process = 1;
	thread_sleep(user_process);
	reclaim_all_user_pages();
	reclaim_all_swap_sections();
	splx(spl);
...
/*
 * Command to print the Debug Flags menu
 */
...
cmd_dbflags(int nargs, char **args)
{
	...
	else if(cmd_dbflagsmenu(nargs, args))
	{
		panic("Error printing dbflags menu at %s:%d\n", __FILE__, __LINE__);
	}
...
/*
 * Command to turn on and off a Debug Flag
 */
static
int
cmd_df(int nargs, char **args)
{
	int toggle_bit = 0; // will hold whether user wants to turn on(1) or turn off(0) that flag
	int dbflag_number; // The Debug Flag number that is to be turned on
	...
	// Sanity check on the flag number
	dbflag_number = atoi(args[1]);
	if((dbflag_number < 1) || (dbflag_number > 12))
	{
		kprintf("Usage: df nr on/off\n");
		return EINVAL;
	}

	// Sanity check on the operation to perform
	if(!strcmp(args[2], "on"))
	{
		toggle_bit = 1;
	}
	...
	if(toggle_bit)
	{
		dbflags = dbflags | (1 << (dbflag_number - 1));
	}
	else
	{
		dbflags = dbflags & ~(1 << (dbflag_number - 1));
...
cmd_dbflagsmenu(int n, char **a)
{
	...
	showmenu("OS/161 Debug Flags menu", dbflagsmenu);
	return 0;
}
