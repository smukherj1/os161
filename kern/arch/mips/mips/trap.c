...
mips_trap(struct trapframe *tf)
{
	...
	switch (code) {
	case EX_MOD:
		result = vm_fault(VM_FAULT_READONLY, tf->tf_vaddr);
		if (result == VM_FAULT_OK) {
			goto done;
		}
		else if(result == VM_FAULT_USER)
		{
		...
		break;
	case EX_TLBL:
		result = vm_fault(VM_FAULT_READ, tf->tf_vaddr);
		if (result == VM_FAULT_OK) {
			goto done;
		}
		else if(result == VM_FAULT_USER)
		...
		break;
	case EX_TLBS:
		result = vm_fault(VM_FAULT_WRITE, tf->tf_vaddr);
		if (result == VM_FAULT_OK) {
			goto done;
		}
		else if(result == VM_FAULT_USER)
...
//	assert((vaddr_t)tf >= curkstack - 1);
//	assert((vaddr_t)tf < curkstack + STACK_SIZE);
}
